...
async def create_comment(
    ...
    return {
        ...
        "author": new_comment.author_id,  # Явно возвращаем author_id
    }
...
async def get_comments(task_id: int, db: Session = Depends(get_db)):
    ...
    formatted_comments = [
        {"id": comment.id, "text": comment.text, "author": comment.author_id}
...
async def get_today_tasks(
    ...
    tasks = (
        ...
        .join(TaskList, Task.task_list_id == TaskList.id, isouter=True)
        .filter(
            Task.notification == True,  # Уведомления включены
            Task.end_date == today,  # Дата завершения = сегодня
            (
                (Task.author_id == current_user_id) |  # Пользователь является автором
                (Task.assigned != None)  # Или пользователь в списке назначенных
            )
        )
        .all()
    )

    # Отбор задач, где пользователь действительно в assigned
    relevant_tasks = [
        task for task in tasks 
        if (
            task.author == current_user_id or 
            str(current_user_id) in (task.assigned.split(",") if task.assigned else [])
        )
    ]
